...
#' @param hits The number of times to reach the shortest tree for each replication.
...
#' @param consense.times Number of times to consense until the consensus is stablilised.
#' @param multiply After hitting target score, find additional trees by fusing suboptimal with optimal trees
#' @param hold.rep The maximum number of trees to retain during each replication.
#' @param keepall Retain all generated trees from each replication regardless of length. This has a different
#' meaning when \code{hits} = 1 and when \code{hits} > 1. When \code{hits} = 1, it is trees from each of the
#' RAS + TBR +  SS or DFT or RAT, in addition to the trees resulting from fusing those.  When \code{hits} > 1,
#' then it means the trees resulting from fusing the initial starting trees for each of starting points.
#' @return A \code{multiPhylo} object.
#' @export
driven <- function(tnt.path, matrix, hold=100, outgroup=NULL, hits=1, replications=4, hold.rep=1,
                   autoconstrain="wagner", rss=TRUE, css=FALSE, xss=FALSE, ratchet.cycles=0,
  ...
  tnt.cmds <- c(tnt.cmds,
                c(paste("hold", hold, ";"),
                  paste("xmult=", "hits", hits, "replications", replications, "hold",
                        hold.rep, paste0(rep("no", !rss), "rss"),
                        paste0(rep("no", !css), "css"),
                        paste0(rep("no", !xss), "xss"),
                        paste0(ifelse(fusing.rounds == 0, "no", ""), "fuse ", ifelse(fusing.rounds == 0, "", fusing.rounds)),
                        paste0(ifelse(ratchet.cycles == 0, "no", ""), "ratchet ", ifelse(ratchet.cycles == 0, "", ratchet.cycles)),
                        paste0(ifelse(drifting.cycles == 0, "no", ""), "drift ", ifelse(drifting.cycles == 0, "", drifting.cycles)),
                        paste0(ifelse(consense.times == 0, "no", ""), "consense ", ifelse(consense.times == 0, "", consense.times)),
                        paste0(ifelse(!multiply, "no", ""), "multiply"),
                        paste0(ifelse(!hybridization, "no", ""), "hybrid"),
                        paste0(ifelse(!keepall, "no", ""), "keepall", ";"))))
