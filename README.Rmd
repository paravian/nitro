...
`create_interface()`, then load a character matrix, configure an analysis, and
run it:
...
`nitro` automatically calculates and returns relevant statistics for the most
parsimonious trees:
...
processed with any compatible command and visualised using the base R graphics
system:
...
Documentation for this package, including function reference pages, vignettes,
and the README, was written with the assistance of a large language model
(Claude, Anthropic). All AI-generated content was reviewed, edited, and
approved by the package author, who takes full responsibility for its accuracy.
